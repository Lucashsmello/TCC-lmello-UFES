...
%   e o segundo, $c_1$, é um BR ligeiramente modificado, que chamaremos de $BR^*$.
  \item Assim como o DBR, é composto de dois BR's, $c_0$ e $c_1$.
  ...
  e $c_1$ trabalha dentro de um novo espaço de características do problema, de nome $X_e$ e
   definido como $X_e=\{0,1\}^{l}$. Assim, $c_0$ e $c_1$ são
  ...
  trabalham com uma dimensão reduzida, em $\{0,1\}^{l-1}$. Digamos que $z=(z_1,z_2,...,z_l)$ seja uma instância de $X_e$, então cada instância 
  de um classificador binário qualquer $c_1^i$ tem $l-1$ características e é definido como sendo $(z_1,...,z_{i-1},z_{i+1},...,z_{l})$.
...
%  \end{itemize}
 ...
 \subsection{Fase de Treinamento}
  A fase de treinamento do \MRLMa~é bastante similar ao DBR.
...
%  \end{equation}
 ...
 resultando em $c_0(x)=\hat{y}^0=(\hat{y}_1^0,\hat{y}_2^0,...,\hat{y}_l^0)$, que servirá de instância, no lugar de $y$,
 para o $c_1$.
 A partir daí, $c_1$ classifica a instância $\hat{y}^0$, gerando a instância $c_1(\hat{y}^0)=\hat{y}^1$, que poderá novamente servir como
 instância para $c_1$. Esse processo de classificação de $c_1$ é repetido para um valor pré-definido $n$ de vezes,
 resultando nas estimativas de rótulos $\hat{y}^1,\hat{y}^2,...,\hat{y}^n$, dentre as quais o último ($\hat{y}^n$) é a classificação final do método
 $C(x)=\hat{y}^n$.
 
 Dessa forma, podemos concluir que \MRLM~é um método recursivo onde
 a aplicação de $c_1$ $2$ vezes sobre
 $c_0(x)$ resulta na estimativa $\hat{y}^2=c_1(c_1(c_0(x)))$,
 a aplicação de $c_1$ $3$ vezes sobre
 $c_0(x)$ resulta na estimativa $\hat{y}^3=c_1(c_1(c_1(c_0(x))))$ e assim por diante.
 Aplicando esse processo recursivo, espera-se que a cada chamada de recursão $i$ a estimativa dos rótulos $\hat{y}^i$ seja melhor do que
